...
async def dial_person(webhook_token, real_number):
    real_number = f"+1{real_number}"
    response = VoiceResponse()
    dial = response.dial(number=real_number, action=f"/ai-assistant/get-call-status/{webhook_token}", timeout=15)
    response = VoiceResponse()
    response.append(dial)
    return HTMLResponse(content=str(response), media_type="application/xml")

def dial_agent(request, user):
    name = user["name"]
    response = VoiceResponse()
    response.say(f"You are being connected to {name}'s AI Assistant")
    host = request.url.hostname
    connect = Connect()
    url=f'wss://{host}/ai-assistant/media-stream/{user["twilio_number"]}'
    print(url)
